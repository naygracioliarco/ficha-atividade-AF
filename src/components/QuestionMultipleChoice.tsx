...
function QuestionMultipleChoice({
  ...
  const keys = (Object.keys(question.options) as ChoiceKey[]).filter(
    (key) => question.options[key] !== undefined && question.options[key] !== ''
  );
        ...
        {keys.map((key) => (
          ...
          >
            ...
            <span style={{ fontFamily: FONTS.primary, color: COLORS.text.primary }}>
              <span style={{ color: COLORS.primary, fontWeight: 'bold' }}>{key}) </span>
              <span dangerouslySetInnerHTML={{ __html: question.options[key] ?? '' }} />
            </span>
            {showResults && selectedAnswer === key && (
              <span
